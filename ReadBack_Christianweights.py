...
#outfile = TFile( "/eos/lhcb/user/b/bkhanji/ForChristian/AllPVs_NewVars/DTT_13512010_Upgrade_withnewiso_CQ.root" , "RECREATE");
outfile = TFile( "/eos/lhcb/user/b/bkhanji/ForChristian/AllPVs_NewVars/DTT_12143001_Upgrade_withnewiso_CQ.root" , "RECREATE");
outfile.cd();
...
BDTreader.AddVariable( "muon_p_Tr_BPVZ", muon_p_Tr_BPVZ );

BDTreader.BookMVA('BDTG','/afs/cern.ch/user/q/qchristi/public/TMVAClassification_BDTG.weights.xml')
...
for event in mychain:
    ...
    for ntrack in range(event.NTr):
        ...
        log_muon_p_Tr_MinIP[0]        = log(mychain.muon_p_Tr_MinIP[ntrack])
        muon_p_Tr_DETA[0]             = mychain.muon_p_ETA - mychain.muon_p_Tr_ETA[ntrack]    
        muon_p_Tr_DPHI[0]             = bool(abs(mychain.muon_p_Tr_PHI[ntrack]-mychain.Bs_PHI)<3.141592653589793)*abs(mychain.muon_p_Tr_PHI[ntrack]-mychain.Bs_PHI)+ bool(abs(mychain.muon_p_Tr_PHI[ntrack]-mychain.Bs_PHI)>3.141592653589793)*abs(mychain.muon_p_Tr_PHI[ntrack]-mychain.Bs_PHI)-3.141592653589793    
        log_muon_p_Tr_PT[0]           = log(mychain.muon_p_Tr_PT[ntrack] )
        log_muon_p_Tr_PAIR_VTCHI2NDOF[0] = log( mychain.muon_p_Tr_PAIR_VTCHI2NDOF[ntrack] )
        ...
        muon_p_Tr_BPVZ[0]             = mychain.muon_p_Tr_BPVZ[ntrack]
